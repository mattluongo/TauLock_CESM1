...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! define variables for overriding: Matt Luongo 05/10/2021
...
!            call seq_hist_spewav('x2a_ax(eai)',gsmap_ax,x2a_ax(eai),atm_nx,atm_ny,1)
      ...
      if (ocn_present .and. ocnrun_alarm) then
         ...
         call seq_timemgr_EClockGetData( EClock_d, curr_ymd=curr_ymd,curr_tod=curr_tod, &
              ...
              calendar=calendar)
         call seq_infodata_GetData( infodata, case_name=case_name) ! get casename string 
         call shr_cal_date2ymd(curr_ymd,yy,mm,dd)
         write(hist_x2o_file,"(a,i4.4,a,i2.2,a,i2.2,a,i5.5,a)") &
                    trim(case_name)//'.cpl.x2o_oo'//'.',yy,'-',mm,'-',dd,'-',curr_tod,'.nc'
         ! write (logunit,*)'Got x2o history file name as above'
         ! write(logunit,*) hist_x2o_file
         call seq_cdata_setptrs(cdata_ox,gsmap=gsmap)
         ! write(logunit,*)'open file'
         call seq_io_wopen(hist_x2o_file,clobber=.true.,cdf64=.true.)
         ! write(logunit,*)'begin writing file'
         call seq_io_write(hist_x2o_file,gsmap,x2o_oo,'x2o_oo')
         ! write(logunit,*)'end writing file'
         call seq_io_close(hist_x2o_file)
         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
