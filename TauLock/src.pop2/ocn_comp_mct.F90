...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  public :: ocn_init_mct
  public :: ocn_run_mct
  public :: ocn_run_ov_mct ! Matt Luongo 05/10/21
...
! !PRIVATE MODULE FUNCTIONS:
  ...
  private :: ocn_import_ov_mct ! Matt Luongo 05/10/21
...
!BOP
!
! !IROUTINE: ocn_run_ov_mct
!
! !INTERFACE:
  subroutine ocn_run_ov_mct( EClock, cdata_o, x2o_o, x2o_ov_o, o2x_o)
!
! !DESCRIPTION:
! Run POP for a coupling interval
!
! !INPUT/OUTPUT PARAMETERS:
    ...
    type(seq_cdata)             , intent(inout) :: cdata_o
    type(mct_aVect)             , intent(inout) :: x2o_o
    type(mct_aVect)             , intent(inout) :: x2o_ov_o ! Matt Luongo 05/10/2021
    type(mct_aVect)             , intent(inout) :: o2x_o
...
!-----------------------------------------------------------------------
    ...
    character(len=*), parameter  :: &
         ! SubName = "ocn_run_mct" 
         SubName = "ocn_run_ov_mct" ! Matt Luongo 05/10/21
...
!-----------------------------------------------------------------------
    ...
    advance: do 
       ...
       if (check_time_flag(cpl_ts) .or. nsteps_run == 0) then
          ! call ocn_import_mct(x2o_o, errorCode)
          call ocn_import_ov_mct(x2o_o, x2o_ov_o, errorCode)   ! Matt Luongo 05/10/2021
...
!***********************************************************************
!BOP
! !IROUTINE: ocn_import_ov_mct
! !INTERFACE:

 subroutine ocn_import_ov_mct(x2o_o, x2o_ov_o, errorCode)
...
!         WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Foxx_taux,n)
!         WORK2(i,j,iblock) = x2o_o%rAttr(index_x2o_Foxx_tauy,n)
         WORK1(i,j,iblock) = x2o_ov_o%rAttr(index_x2o_Foxx_taux,n) ! Matt Luongo 05/10/2021
         WORK2(i,j,iblock) = x2o_ov_o%rAttr(index_x2o_Foxx_tauy,n) ! Matt Luongo 05/10/2021
...
!-----------------------------------------------------------------------
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
!         'ocn_import_mct: error in update_ghost_cells_coupler_fluxes')
         'ocn_import_ov_mct: error in update_ghost_cells_coupler_fluxes') ! Matt Luongo 05/10/2021
      return
   endif
...
!-----------------------------------------------------------------------
   ...
   if (index_x2o_Sa_co2prog > 0) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
!            'ocn_import_mct: error updating PROG CO2 halo')
            'ocn_import_ov_mct: error updating PROG CO2 halo') ! Matt Luongo 05/10/2021
         return
      endif
   ...
   if (index_x2o_Sa_co2diag > 0) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
!            'ocn_import_mct: error updating DIAG CO2 halo')
            'ocn_import_ov_mct: error updating DIAG CO2 halo') ! Matt Luongo 05/10/2021
         return
      endif
...
!EOC
 ...
 end subroutine ocn_import_ov_mct
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
