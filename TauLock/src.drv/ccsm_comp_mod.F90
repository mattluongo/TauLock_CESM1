...
!-------------------------------------------------------------------------------
   ...
   use ocn_comp_mct, only: ocn_init_mct, ocn_run_mct, ocn_run_ov_mct, ocn_final_mct ! Matt Luongo 05/11/2021
...
#include <mpif.h>
   ...
   integer  :: dtimeo                 ! dt of one ocn coupling interval
   integer  :: dtimer                 ! dt of one rof coupling interval
   integer  :: dtimea                 ! dt of one atm coupling interval
...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! define variables for overriding: Matt Luongo 05/11/21
   character(CL)           :: hist_x2o_file ! local path to x2o hist file
   ...
   ! below line based on code from seq_avdata_mod.F90 ! Matt Luongo 05/11/2021
   type(mct_aVect) :: x2o_ov_oo(num_inst_ocn)    ! Ocn import, ocn grid, ocn pes - allocated in ocn gc
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!   endif
   ...
   ! Check some coupling intervals, dependent on compset

   call seq_timemgr_EClockGetData( EClock_a, dtime=dtimea)
   call seq_timemgr_EClockGetData( EClock_o, dtime=dtimeo)
   call seq_timemgr_EClockGetData( EClock_r, dtime=dtimer)
   if (rof_present .and. ocnrof_prognostic .and. dtimeo < dtimer) &
      call shr_sys_abort('ocn and rof coupling periods inconsistent')
   ...
   if (iamin_CPLID) then
      ...
      do eoi = 1,num_inst_ocn
         ...
         call mct_aVect_init(x2o_ov_oo(eoi), rList=seq_flds_x2o_fields, lsize=lsize_o) ! Matt Luongo 05/11/2021
         call mct_aVect_zero(o2x_ax(eoi))
         call mct_aVect_zero(o2x_ix(eoi))
         call mct_aVect_zero(o2x_wx(eoi))
         call mct_aVect_zero(x2o_ov_oo(eoi)) ! Matt Luongo 05/11/2021
      enddo
      ...
      if (ocn_present .and. ocnrun_alarm) then
         do eoi = 1,num_inst_ocn
            if (iamin_OCNID(eoi)) then
               ...
               ! Uncomment below to offset input wind stress by 1 year
                   ! yy = yy + 1
               ! Read in hist x2o file
               ! be sure to plug om relevant username and output case name/directory to be read in
               write(hist_x2o_file,"(a,i4.4,a,i2.2,a,i2.2,a,i5.5,a)") &
                   '/glade/scratch/USER/OUTPUTCASENAME/run/OUTPUTCASENAME.cpl.x2o_oo.',yy,'-',mm,'-',dd,'-',curr_tod,'.nc'
               call seq_cdata_setptrs(cdata_ox,gsmap=gsmap)
               call seq_io_read(hist_x2o_file,gsmap_ox,x2o_ov_oo,'x2o_oo')
               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               ! run pop2 while piping in x2o_ov with edited subroutine
               !call ocn_run_mct( EClock_o, cdata_oo(eoi), x2o_oo(eoi), o2x_oo(eoi))
               call ocn_run_ov_mct( EClock_o, cdata_oo(eoi), x2o_oo(eoi), x2o_ov_oo(eoi), o2x_oo(eoi))
               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!     
